...
-- Add RLS policies for the organization_public_info table
-- Currently has no RLS protection
ALTER TABLE public.organization_public_info ENABLE ROW LEVEL SECURITY;

-- Only allow authenticated users to view basic org info they're part of
CREATE POLICY "Users can view public org info for their organization" 
ON public.organization_public_info 
FOR SELECT 
USING (
  id IN (
